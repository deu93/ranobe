        ...
        <div class="w-8/12  ml-5 mr-5">
            ...
            <div class="bg-white w-12/12 mb-4 pl-3 pr-2 m-2 ml-4 ">
                ...
                <div class="w-full flex justify-center">
                 ...
                 <a type='submit' href="{{ url('read-chapter/' . $book->id) }}" class="bg-blue-500 mt-5 ml-3 text-white px-4 no-underline py-3 mb-3  rounded font-medium w-3/12 ">Читать с 1 главы</a>
                 @auth()
                     @if (auth()->user()->role > 0 && auth()->user()->id == $book->user_id )
                     <a href="{{ route('add-chapter', $book->id) }}" class="bg-blue-500 mt-5 ml-5 text-white px-4 no-underline py-3 mb-3  rounded font-medium w-3/12 ">Добавить главы</a>
                        <form action="{{ url('delete-book/' . $book->slug) }}" method="POST">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="bg-red-500 mt-2 text-white px-4 no-underline ml-3 py-3 mb-3  rounded font-medium w-3/12 ">Удалить</button>
                        </form>
                     @endif
                 @endauth
             </div>
